...
# their build-time GeneratePackageOnBuild wiring — pack-sql.sh packs each directly with
# that disabled. Output goes straight to the local feed, in dependency order: schema
# packages reference each other at $(Version) (Notifications -> Security -> Users), so
# each must resolve from the feed (RestoreAdditionalProjectSources) while its dependents build.
echo ""
echo -e "${YELLOW}Packing SQL schema (.Mssql) packages...${NC}"
"$REPO_ROOT/builds/pack-sql.sh" "$LOCAL_NUGET_DIR" \
    --configuration "$CONFIGURATION" \
    /p:Version="$PACKAGE_VERSION" \

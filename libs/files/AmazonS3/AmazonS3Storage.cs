...
public class AmazonS3Storage : IFileStorage
{
    ...
    public async Task<File?> DeleteFileAsync(File? file, CancellationToken? token = null)
    {
        if (file == null) return null;
        var (bucket, key) = GetBucketAndKey(file.Path);
        var req = new DeleteObjectRequest { BucketName = bucket, Key = key };
        await Client.DeleteObjectAsync(req, token ?? CancellationToken.None);
        return file;
    }

    private async Task<long> WriteStreamToS3Async(File file, Stream stream, CancellationToken? token = null)
    {
        var (bucket, key) = GetBucketAndKey(file.Path);
        ...
        var resp = await Client.PutObjectAsync(putReq, token ?? CancellationToken.None);

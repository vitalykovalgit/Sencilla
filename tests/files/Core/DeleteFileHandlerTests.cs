...
///
/// Covers: single file deletion, deletion with variants, non-existent file, event publishing, resolution file deletion.
/// </summary>
public class DeleteFileHandlerTests
{
    ...
    private DeleteFileHandler CreateHandler() =>
        new(_storage.Object, _events.Object, _pathResolver.Object, _readRepo.Object, _deleteRepo.Object);
    ...
    [Fact]
    public async Task Handle_FileWithResolutions_DeletesAllResolutionFiles()
    {
        var fileId = Guid.NewGuid();
        var file = new File
        {
        ...
        _pathResolver.Setup(p => p.GetResolutionPath(file, 100)).Returns("test/photo_100.jpg");
        _storage.Setup(s => s.DeleteFileAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
        ...
        Assert.Equal(StatusCodes.Status204NoContent, context.Response.StatusCode);
        _storage.Verify(s => s.DeleteFileAsync("test/photo_600.jpg", It.IsAny<CancellationToken>()), Times.Once);
        _storage.Verify(s => s.DeleteFileAsync("test/photo_100.jpg", It.IsAny<CancellationToken>()), Times.Once);
        _storage.Verify(s => s.DeleteFileAsync(file, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_FileWithNoRes_DoesNotCallDeleteResolutionFiles()
    {
        var fileId = Guid.NewGuid();
        var file = new File { Id = fileId, Name = "photo.jpg", Path = "test/photo.jpg" };
        var context = CreateDeleteContext(fileId);
        ...
        _storage.Verify(s => s.DeleteFileAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never);
        _pathResolver.Verify(p => p.GetResolutionPath(It.IsAny<File>(), It.IsAny<int>()), Times.Never);
    }
}
